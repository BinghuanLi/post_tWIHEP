...
// Function: 
// select prompt MC flip events according to Regions/Postfix and save/reconstruct only interested branches
...
// same sign lepton : fabs(Sum2lCharge)==0 
// dilep are tight tight: Dilep_nTight ==2 
// at least one electron : Dilep_pdgId > 1
//      Region cuts : 
...
// options
Bool_t _useChargeMis = true; // set to true if we recalculate ChargeMis Weight.


// ChargeMis
//Histograms that are used for applying charge mismeasurement weight
TH2F* _chargeMis;
TString chargeMisFileName = "/publicfs/cms/data/TopQuark/cms13TeV/Binghuan/tWIHEPFramework/config/weights/ttH2018/ElectronChargeMisIdRates_2017_2018Jun22.root";
TString chargeMisHistName = "eChargeMisIdRates";

void Rootplas_Prompt_flip(TString InputDir, TString OutputDir, TString FileName, TString Postfix){
    ...
    TString Output = OutputDir  +"/"+FileName+ "_Promptflip_"+Postfix+".root";
    std::cout <<" inpupt file is " << Input << std::endl;
    std::cout <<" outpupt file is " << Output << std::endl;

    if(_useChargeMis){
        std::cout<< " re calculate chargeMis " << std::endl;    
        setChargeMisHistograms(chargeMisFileName, chargeMisHistName, &_chargeMis);
    }
    ...
    float lep1_charge(0.), lep2_charge(0.), lep3_charge(0.);
    ...
    oldtree->SetBranchAddress("lep3_charge", &lep3_charge);
    ...
    for (Long64_t i=0;i<nentries; i++) {
        ...
        if(!(fabs(Sum2lCharge)==0))pass2LCharge = kFALSE;
        if(!(Dilep_nTight ==2))pass2LTightID = kFALSE;
        if(!(Dilep_pdgId >1))pass2LpdgId = kFALSE;
        ...
        passCut = pass2LMatchRightCharge && pass2LPromptFS && pass2LCharge && pass2LTightID && (pass2LRegionCut || passTHSelectionCut) && pass2LpdgId;
        ...
        if(passCut){
            ...
            if(_useChargeMis){
               ...
               std::tie(ChargeMis,ChargeMis_SysUp,ChargeMis_SysDown) = getChargeMisWeight( lep1_pdgId, lep1_charge,  lep1_conept,  lep1_eta,  lep2_pdgId, lep2_charge,  lep2_conept,  lep2_eta, _chargeMis);
