...
TString input_json_file = "/publicfs/cms/data/TopQuark/cms13TeV/Binghuan/tWIHEPFramework/config/weights/DNN/neural_net_weights.json";
...
void setDNNflag(std::vector<double> DNN_vals, float& DNN_maxval, float& DNNCat, float& DNNSubCat1, float& DNNSubCat2, float Dilep_pdgId, float lep1_charge){
    ...
    if(Dilep_pdgId==3){//ee
        ...
        DNNSubCat1 = Dilep_pdgId>1.5? 3:8;// em:mm
    }else if(DNNCat==2){// ttJnode
        DNNSubCat1 = Dilep_pdgId>1.5? 4:9;// em:mm
    }else if(DNNCat==3){// ttWnode
        DNNSubCat1 = Dilep_pdgId>1.5? 5:10;// em:mm
    }else if(DNNCat==4){// ttZnode
        DNNSubCat1 = Dilep_pdgId>1.5? 6:11;// em:mm
    }else if(DNNCat==5){// tHnode
        DNNSubCat1 = Dilep_pdgId>1.5? 7:12;// em:mm
    }else{
