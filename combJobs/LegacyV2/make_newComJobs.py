...
def prepareCshJob(shFile, dirName, datacardName, year):
    ...
    if makeWS:
        ...
        convertCards =  "text2workspace.py {}/{}.txt -o {}_3poi.root -P HiggsAnalysis.CombinedLimit.PhysicsModel:multiSignalModel --PO verbose --PO 'map=.*/TTW.*:r_ttW[1,0,6]' --PO 'map=.*/TTWW.*:r_ttW[1,0,6]' --PO 'map=.*/ttH.*:r_ttH[1,-1,3]' --PO 'map=.*/tH.*:r_tH[1,-40,40]'".format(dirName,datacardName,datacardName)
