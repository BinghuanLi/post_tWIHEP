...
######### to run #########
#python make_DNNinput_comparisons.py -y 2017 -d TTH --dataset2 TTW --dataset3 THQ --dataset4 TTJ --createROOT -i /home/binghuan/Work/TTHLep/TTHLep_RunII/Legacy_NNs/data/LegacyMVA_tau2p1_1204/DiLepRegion/TR_SR_RunII/ -o /home/binghuan/Work/TTHLep/TTHLep_RunII/Legacy_NNs/plots/LegacyNN_plotPHD/
##########################
...
if dataset4 != "NULL":
    Color={
     "%s_SR_%i"%(dataset,year):kBlack,"%s_SR_%i"%(dataset2,year):kRed,"%s_SR_%i"%(dataset3,year):kBlue, "%s_SR_%i"%(dataset4,year):kViolet,
     "%s_TR_%i"%(dataset,year):kBlack,"%s_TR_%i"%(dataset2,year):kRed,"%s_TR_%i"%(dataset3,year):kBlue, "%s_TR_%i"%(dataset4,year):kViolet
    }
    Style={
     "%s_SR_%i"%(dataset,year):1,"%s_SR_%i"%(dataset2,year):1,"%s_SR_%i"%(dataset3,year):1, "%s_SR_%i"%(dataset4,year):1,
     "%s_TR_%i"%(dataset,year):2,"%s_TR_%i"%(dataset2,year):2,"%s_TR_%i"%(dataset3,year):2, "%s_TR_%i"%(dataset4,year):2
    }
    sampleName = [
        "%s_SR_%i"%(dataset,year),"%s_SR_%i"%(dataset2,year), "%s_SR_%i"%(dataset3,year),"%s_SR_%i"%(dataset4,year),
...
def getSeparation(hsig, hbkg):
    ...
    for i in range(1,nBins):
        sig_bin = hsig.GetBinContent(i)
        bkg_bin = hbkg.GetBinContent(i)
        # Separation:
...
def plotSysts():
    ...
    for feature, values in features.items():
        ...
        legend = TLegend(0.45,0.6,0.95,0.98)
        ...
        c.SaveAs("%s%s_%s_DNN_inputs_overlay.png"%(outputdir,feature,plotname))
