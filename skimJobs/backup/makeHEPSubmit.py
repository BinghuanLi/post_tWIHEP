...
outputBaseDir = "/publicfs/cms/data/TopQuark/cms13TeV/Binghuan/ttH2019/condorStuff/rootplasV1_20190212/"
...
def prepareCshJob(sample,shFile,inputDir,outputDir, sampleName):
        ...
        if process in samples94XData:
            ...
            for region in Regions:
                ...
                print >> subFile, "root -b -q -l "+executable_Prompt_flip+"'(\""+inputDir+'","'+outputDir+'","'+sample+'","'+region+'"'+")'"
            ...
            for region in Regions:
                ...
                print >> subFile, "root -b -q -l "+executable_Prompt_fake+"'(\""+inputDir+'","'+outputDir+'","'+sample+'","'+region+'"'+")'"
                print >> subFile, "root -b -q -l "+executable_Prompt_flip+"'(\""+inputDir+'","'+outputDir+'","'+sample+'","'+region+'"'+")'"
        #root -l -b -q Rootplas_ttH2L.C'("/publicfs/cms/data/TopQuark/cms13TeV/Binghuan/ttH2019/condorStuff/20190130/ttH2L/ttHnobb/skims","/publicfs/cms/data/TopQuark/cms13TeV/Binghuan/ttH2019/condorStuff/20190130/ttH2L/ttHnobb/rootplas/rootplas","ttHnobb1","rootpas_ttH2L",1)'
        elif process in samples94XConv:
            for region in Regions:
                print >> subFile, "root -b -q -l "+executable_Conv_2lss+"'(\""+inputDir+'","'+outputDir+'","'+sample+'","'+region+'"'+")'"
        else:
            print ("ERROR '"+process+"' process not found in samples94XData, samples94XConv, samples94XPrompt")
        subprocess.call("chmod 777 "+shFile, shell=True)
        ...
        for sample in samples:
            ...
            if sampleName in samples94XData:
                ...
                for region in Regions:
                    ...
                    print >> MergeFile, "hadd -f "+outputBaseDir + "/" + analysis + regiondir + "/"+ sampleName + "/skims/merged"+sampleName+"_Promptflip_"+region+".root  "+ outputBaseDir + "/" + analysis + regiondir + "/"+ sampleName + "/skims/"+sampleName+"*_Promptflip_"+region+".root"
                ...
                print ("ERROR '"+sampleName+"' sampleName not found in samples94XData, samples94XConv, samples94XPrompt")
    for region in Regions:
        MergeFileName = analysis+regiondir+region+"merge.sh"
        MergeFile = MergeFiles[MergeFileName]
